...
public class StatisticalServiceImpl implements StatisticalService{
    ...
    public List<StoreStatistical> findAll() {
        ...
        for (Store s:storeList) {
            ...
            storeStatistical.setStoreAddress(s.getHomeNumber()+", "+s.getStreet()+", "+s.getDistrict()+", "+s.getCity());
    ...
    @Override
    public Page<StoreStatistical> getAll(Integer pageNo) {
        List<StoreStatistical> storeStatisticalList = new ArrayList<>();
        List<Store> storeList =  storeRepository.findAll();
        for (Store s:storeList) {
            List<Orders> ordersList=orderRepository.findAllByStore(s);
        ...
        storeStatisticalList.sort((o1, o2) -> o1.getOrderTotal() - o2.getOrderTotal());
//        Sort sort = Sort.by(Sort.Direction.ASC, "orderTotal");
        Pageable pageable = PageRequest.of(pageNo - 1, 5);
        int start = (int) pageable.getOffset();
        int end = (int) ((pageable.getOffset() + pageable.getPageSize()) > storeStatisticalList.size() ? storeStatisticalList.size() : pageable.getOffset() + pageable.getPageSize());
        storeStatisticalList = storeStatisticalList.subList(start, end);
        return new PageImpl<>(storeStatisticalList, pageable, storeList.size());
    }
    ...
    @Override
    public Page<ProductStatistical> getAllProduct(Integer pageNo) {
        List<ProductStatistical> productStatisticalList = new ArrayList<>();
        List<Product> productList = productRepository.findAll();
        ...
        Pageable pageable = PageRequest.of(pageNo - 1, 5);
        int start = (int) pageable.getOffset();
        int end = (int) ((pageable.getOffset() + pageable.getPageSize()) > productStatisticalList.size() ? productStatisticalList.size() : pageable.getOffset() + pageable.getPageSize());
        productStatisticalList = productStatisticalList.subList(start, end);
        return new PageImpl<>(productStatisticalList, pageable, productList.size());
    }
